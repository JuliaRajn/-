...
`    `# Настройте подписи данных

`  `fig.data[0].hoverinfo = 'text'

`  `fig.data[0].text = ['Текст всплывающей подсказки для точки 1', 'Текст всплывающей подсказки для точки 2']
...
`    `# Добавьте кнопки для фильтрации данных

ChatGPT, [13.01.2025 17:16]

fig.update\_layout(updatemenus=[dict(buttons=[dict(label='Показать все', method='update', args=[{'visible': [True, True, True]}])])])
